...
public class Grafico extends javax.swing.JComponent {
    ...
    private void createChart(Graphics grphcs) {
        ...
        if (!list.isEmpty()) {
            ...
            for (ModeloGrafico data : list) {
                g2.setColor(data.getColor());
                double angle = valuesToAngle(data.getValues());
                double rs = valuesToRealSize(data.getValues(), size);
    ...
    private void calculateValues(ModeloGrafico data) {
        ...
        totalValues = 0;
        for (ModeloGrafico l : list) {
            totalValues += l.getValues();
        }
    }

    public void addItem(ModeloGrafico data) {
        list.add(data);
        calculateValues(data);
        repaint();
        GraficoTexto label = new GraficoTexto();
